...
export class QuoteService {
  ...
  queryByStock(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);

    return this.http
      .get<IQuote[]>(this.applicationConfigService.getEndpointFor('api/stock-quotes'), { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient(quote: IQuote): IQuote {
    return Object.assign({}, quote, {
      date: quote.date?.isValid() ? quote.date.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((quote: IQuote) => {
        quote.date = quote.date ? dayjs(quote.date) : undefined;
      });
    }
    return res;
  }
